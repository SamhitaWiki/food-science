...
for i in range(len(day)):
    if (day[i] == 'Today') or (day[i]== 'Yesterday'):
        if day[i] == 'Today':
            d = (datetime.datetime.today())
            recipe_date.append(d.strftime('%d/%m/%Y'))
            print(d.strftime('%d/%m/%Y'))
        else :
            today = datetime.date.today()
            yesterday = today - datetime.timedelta(days = 1)
            print(yesterday.strftime('%d/%m/%Y'))
            recipe_date.append(yesterday.strftime('%d/%m/%Y'))
    else:
        d = datetime.datetime.strptime(day[i], '%d %b %y').strftime('%d/%m/%Y')
        recipe_date.append(d)
        print(d)

last_date = '04/05/2019'

for i in range(len(recipe_date)):
    last = datetime.datetime.strptime(last_date, "%d/%m/%Y")
    d = datetime.datetime.strptime(recipe_date[i], "%d/%m/%Y")
    if d < last :
        break
